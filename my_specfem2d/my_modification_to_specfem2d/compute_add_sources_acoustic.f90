        ...
        if (source_type(i_source) == 1) then
          ...
          do j = 1,NGLLZ
            do i = 1,NGLLX
              ...
              !print *, 'Hey, iglob', iglob 
              ! old way: source without factor 1/kappa
              !potential_dot_dot_acoustic(iglob) = potential_dot_dot_acoustic(iglob) - &
              !                                    sourcearrays(i_source,1,i,j) * stf_used
              !print *, 'Hey, jacobian(i,j,ispec) ,i,j', jacobian(i,j,ispec),i,j
              !!!xgamma = - xiz(i,j,ispec) * jacobian(i,j,ispec)
              !!!zgamma = + xix(i,j,ispec) * jacobian(i,j,ispec)
              !!!jacobian1D = sqrt(xgamma**2 + zgamma**2)
              
              !print *, 'Hey, jacobian1D ',jacobian1D, i,j
              !nx = - zgamma / jacobian1D
              !nz = + xgamma / jacobian1D
              !weight = jacobian1D * wzgll(j)
              !print *, 'Hey, weight ', weight, i,j
              !potential_dot_dot_acoustic(iglob) = potential_dot_dot_acoustic(iglob) - &
              !                    real(sourcearrays(i_source,1,i,j) * weight * stf_used, kind=CUSTOM_REAL)
...
!              nx = + zxi / jacobian1D
!              nz = - xxi / jacobian1D
!              print *, 'Hey, nx', nx, 'nz', nz
!              weight = jacobian1D * wxgll(i)
!              !print *, 'Hey, weight ', weight, i,j
!
!              !print *, 'Hey, sourcearrays(i_source,1,i,j)', sourcearrays(i_source,1,i,j), i,j
!              potential_dot_dot_acoustic(iglob) = potential_dot_dot_acoustic(iglob) + &
!                                  real(weight * stf_used, kind=CUSTOM_REAL)
!              
!
!              !ZN becareful the following line is new added, thus when do comparison
!              !ZN of the new code with the old code, you will have big difference if you
